...
export const createFlyingLine = (
  ...
  // Calculate path length and adjust number of points
  const pathLength = Vector3.Distance(start, end)
  const pointsPerUnit = 40 // Density of points
  const numPoints = Math.min(1000, Math.max(200, Math.floor(pathLength * pointsPerUnit)))
  ...
  const calculatePoints = () => {
    ...
    for (let i = 0; i <= numPoints; i++) {
      // Use cubic ease-in-out for smoother distribution
      const t = i / numPoints
      const oneMinusT = 1 - t
      points.push(
        new Vector3(
