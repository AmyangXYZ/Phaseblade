...
function BjsScene({
  ...
  useEffect(() => {
    ...
    shadowGeneratorRef.current = new ShadowGenerator(
      4096,
      directionalLight,
      true
    );
    ...
    shadowGeneratorRef.current.filteringQuality =
    ...
    scene.onPointerDown = (e) => {
      if (e.button === 0 || e.button === 2) {
        ...
        if (pickResult.hit && pickResult.pickedMesh) {
          if (
            pickResult.pickedMesh.name.includes("-") &&
            !pickResult.pickedMesh.name.startsWith("section_")
          ) {
            return;
          }
        }
      ...
      nodes.forEach((node) => {
        ...
        nodeMesh.actionManager.registerAction(
          new ExecuteCodeAction(ActionManager.OnPickTrigger, () => {
            ...
            if (selectedNodeRef.current !== node.id) {
              camera.spinTo(
                nodeMesh.position.clone().add(new Vector3(0, 24, -12)),
        ...
        if (!unit.isStatic) {
          ...
          const animate = (currentTime: number) => {
            ...
            while (accumulator >= targetDelta) {
              ...
              const newX =
                nodeMesh.position.x +
                (Math.sin(time * 1.1) / 2) * randomOffset.x;
              const newZ =
                nodeMesh.position.z +
                (Math.sin(time * 1.2) / 2) * randomOffset.z;
